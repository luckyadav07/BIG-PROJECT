      ...
      <div className="flex items-center justify-between border-b pb-4" style={{ borderColor: "var(--border-color)" }}>
        ...
        {messages.length > 0 && (
          <button
            ...
            className="flex items-center gap-1.5 px-3 py-1.5 text-xs font-bold rounded-xl border transition cursor-pointer hover:bg-black/5 dark:hover:bg-white/5"
      ...
      <div className="grid lg:grid-cols-4 gap-6 h-[calc(100vh-14rem)] items-stretch">
          ...
          <div className="flex-1 overflow-y-auto p-4 md:p-6 space-y-2">
            <AnimatePresence>
              {messages.length === 0 ? (
                ...
                >
                  ...
                  <motion.h2
                    variants={itemVariants}
                    className="text-xl md:text-2xl font-extrabold tracking-tight text-center"
                  ...
                  <motion.p
                    variants={itemVariants}
                    className="text-xs md:text-sm mt-1.5 mb-8 max-w-md text-center leading-relaxed"
                    ...
                    {STARTER_CARDS.map((card) => {
                      ...
                      return (
                        <motion.div
                          ...
                          className="p-4 rounded-xl border cursor-pointer flex gap-3 text-left transition-all duration-200 hover:bg-black/[0.01] dark:hover:bg-white/[0.01]"
                        ...
                        >
                          ...
                          <div>
                            <h3 className="text-xs font-bold flex items-center gap-1" style={{ color: "var(--text-primary)" }}>
                              {card.title}
            ...
            {typing && (
              ...
              >
                ...
                <div 
                  className="rounded-2xl rounded-tl-sm px-4 py-3.5 flex items-center gap-1.5 shadow-sm border"
                  style={{ background: "var(--bg-elevated)", borderColor: "var(--border-color)" }}
                >
            ...
            <div className="px-4 pb-2.5 overflow-x-auto scrollbar-none flex gap-2">
              {SUGGESTED_PROMPTS.map((p) => (
                <button
                  ...
                  className="shrink-0 rounded-full border px-3.5 py-1.5 text-xs font-semibold hover:border-accent/20 transition duration-200 cursor-pointer hover:bg-black/5 dark:hover:bg-white/5"
                  style={{ background: "var(--bg-elevated)", borderColor: "var(--border-color)", color: "var(--text-secondary)" }}
                >
                  {p}
                </button>
        ...
        <div className="space-y-4 hidden lg:block">
          ...
          <Card className="border relative overflow-hidden" style={{ background: "var(--bg-card)", borderColor: "var(--border-color)" }}>
            ...
            <h3 className="font-bold text-sm flex items-center gap-1.5 mb-2.5" style={{ color: "var(--text-primary)" }}>
          ...
          <Card className="border" style={{ background: "var(--bg-card)", borderColor: "var(--border-color)" }}>
            <h3 className="font-bold text-sm mb-3" style={{ color: "var(--text-primary)" }}>Optimize Resume format</h3>
            <Link to="/resume-analyzer">
              <button
                className="flex flex-col items-center justify-center gap-2.5 w-full rounded-xl border border-dashed px-4 py-6 text-xs transition-all duration-200 cursor-pointer hover:border-accent/30"
                style={{ background: "var(--bg-elevated)", borderColor: "var(--border-color)", color: "var(--text-secondary)" }}
              >
          ...
          <Card className="border" style={{ background: "var(--bg-card)", borderColor: "var(--border-color)" }}>
            <h3 className="font-bold text-sm mb-3 flex items-center gap-1.5" style={{ color: "var(--text-primary)" }}>
            ...
            <div className="space-y-2 text-xs">
              {[
                ...
                <button
                  ...
                  className="block w-full text-left transition duration-200 py-1.5 border-b last:border-b-0 font-semibold truncate hover:text-accent"
